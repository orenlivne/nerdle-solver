...
def all_answers(num_slots: int, debug: bool = False) -> List[str]:
    ...
    # If num_slots is odd, we have the corner case of X=X expressions with no
    # ops.
    if num_slots % 2 == 1:
        num_result_slots = num_slots // 2
        result_range = (0 if num_result_slots == 1 else 10 **
                        (num_result_slots - 1), 10 ** num_result_slots)
    ...
    for num_param in range(3, num_slots - 1):
        ...
        for num_ops in range(1, (num_param - 1) // 2 + 1):
            ...
            for op_slot in op_slots:
                param_lens = [(n - 1)
                              for n in diff((-1, ) + op_slot + (num_param, ))]
                if debug:
                    print("\t\t", "o".join("X" * n for n in param_lens) +
                          " = " + "X" * (num_slots - num_param - 1))
